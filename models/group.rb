class Group
  ...
  def self.enablable
    %w{teams qualities timetables rotas tiers accommodation transport bookings wishlists budget}
  end  
  ...
  def send_email
      ...
      html_part = Mail::Part.new do
        ...
        body %Q{#{group.account.name} (#{group.account.email}) created a new group: <a href="#{ENV['SCHEME']}://#{ENV['DOMAIN']}/h/#{group.slug}">#{group.name}</a>}
      end
